...
export default function MyOrders() {
  ...
  useEffect(() => {
    const ordersRaw = typeof window !== "undefined" ? window.localStorage.getItem("orders") : null;
    const lastOrderRaw = typeof window !== "undefined" ? window.localStorage.getItem("lastOrder") : null;

    let list: Order[] = [];
    if (ordersRaw) {
      try { list = JSON.parse(ordersRaw) as Order[]; } catch { /* ignore */ }
    }
    if (lastOrderRaw) {
      try {
        const last = JSON.parse(lastOrderRaw) as Order;
        if (last?.id && !list.some((o) => o.id === last.id)) list.push(last);
      } catch { /* ignore */ }
    }
              ...
              <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                <Stack spacing={1.2}>
                  ...
                  <Stack direction="row" justifyContent="space-between">
                    ...
                    <Typography variant="body2">${Number(order.orderAmount).toFixed(2)}</Typography>
                  </Stack>
